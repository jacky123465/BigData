...
class RCNN(BaseModel):
    ...
    def __init__(self, sequence_length, num_classes, vocab_size,
                 ...
                 hidden_size, l2_lambda, grad_clip, context_size, activation_func, CR_E,
        ...
        self.logits, self.outpooling = self.inference()      # ? * 7
        self.loss_val = self.loss()
        self.train_op = self.train()
        self.predictions = tf.argmax(self.logits,axis = 1,name = 'predictions')
        # self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.input_y, 1), tf.argmax(self.logits, 1)), tf.float32))
    ...
    def instantiate_weight(self):
        with tf.name_scope('weight'):
            ...
            self.Embedding = tf.Variable(tf.constant(self.CR_E, shape=[np.shape(self.CR_E)[0], np.shape(self.CR_E)[1]]), "CR_E")
    ...
    def loss(self):
        with tf.name_scope('loss'):
            ...
            out = tf.exp(self.logits) / (1 + tf.exp(self.logits))
            cost_tmp = tf.multiply(out, self.input_y) + (1 - self.input_y)
            cost_tmp0 = 1 - tf.multiply(out, 1 - self.input_y)
            cost = tf.reduce_sum(tf.log(cost_tmp)) + tf.reduce_sum(tf.log(cost_tmp0))
            # cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=out, labels=y))
            cost = -cost / 50
            loss = cost + (0.00003/2) * tf.reduce_sum(tf.multiply(self.W_l, self.W_l)) + (0.000003/2) * tf.reduce_sum(tf.multiply(self.W_r, self.W_r)) + \
                   (0.0001/2) * tf.reduce_sum(tf.multiply(self.W_sl, self.W_sl)) + (0.0001/2) * tf.reduce_sum(tf.multiply(self.W_sl, self.W_sl))
